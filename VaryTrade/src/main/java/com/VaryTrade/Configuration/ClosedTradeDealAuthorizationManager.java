...
@Component
public class ClosedTradeDealAuthorizationManager implements AuthorizationManager<RequestAuthorizationContext> {
	@Autowired
	...
	public AuthorizationDecision check(Supplier<Authentication> authentication, RequestAuthorizationContext context) {
		...
		if(!authority.equals("ROLE_USER")) {
			...
			int closedTradeDealId = Integer.valueOf(context.getRequest().getParameter("id"));
			ClosedTradeDeal closedTradeDeal = appTradeDealRepository.retrieveClosedTradeDealById(closedTradeDealId);
			boolean authorize = closedTradeDeal != null && appTradeDealService.doesClosedTradeDealBelongToCollector(closedTradeDeal, 
